...
function buildProjectLookupKeys(project) {
  const projectId = collapseWhitespace(project?.id);
  const projectSlug = collapseWhitespace(project?.slug);
  const projectName = normalizeText(project?.name);
  const keys = [projectId, projectSlug, projectName, 'default']
...
function resolveFallbackServices(project) {
  const configuredFallbacks = parseProjectServicesFallbackEnv();
  const lookupKeys = buildProjectLookupKeys(project);
...
function loadProjectServices(project) {
