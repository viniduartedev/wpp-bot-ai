...
function normalizeServiceEntry(rawService, index) {
  ...
  return {
    ...
    label,
    aliases,
    ...(typeof rawService.order === 'number' ? { order: rawService.order } : {}),
...
function isActiveService(service) {
  ...
  return service?.active !== false && service?.isActive !== false && normalizedStatus !== 'inactive';
}
...
async function loadFirestoreServices(project, options = {}) {
  const resolvedTenantSlug = ACTIVE_TENANT_SLUG;

  const { botDb, firebaseProjectId } = require('../firebase-admin').getFirestoreClients();
  ...
  const rawServices = snapshot.docs
    ...
    .sort(sortServicesByCatalogOrder);
...
async function loadRuntimeProjectServices(project, options = {}) {
  ...
  try {
    const firestoreCatalog = await loadFirestoreServices(project, {
      tenantSlug: resolvedTenantSlug,
    });

    if (firestoreCatalog && firestoreCatalog.services.length > 0) {
      return firestoreCatalog;
    }

    if (firestoreCatalog) {
      console.warn('[services] Nenhum servico ativo encontrado na colecao services para o tenant unico.', {
        projectId: project?.id || null,
        projectSlug: project?.slug || null,
    ...
    console.warn('[services] Falha ao carregar servicos do botDb. Aplicando fallback legado.', {
      projectId: project?.id || null,
      projectSlug: project?.slug || null,
      tenantSlug: resolvedTenantSlug || null,
      message: error.message,
  ...
  return loadProjectServices(project, {
    tenantSlug: resolvedTenantSlug,
  });
}
