...
async function findFallbackConnectionByTenantSlug({ tenantSlug, to }) {
  ...
  const discardedConnections = connections.filter((connection) => isConnectionInactive(connection));

  console.log('[routing] fallbackLookup candidates', {
    to: normalizedTo,
    tenantSlug: normalizedTenantSlug,
    totalFound: connections.length,
    totalActive: activeConnections.length,
    runtimeEnvironment: process.env.BOT_RUNTIME_ENV?.trim() || null,
    found: connections.map((connection) =>
      sanitizeConnectionForLog(connection, {
        targetProjectId: connection.projectId || connection.targetProjectId || null,
      }),
    ),
    discarded: discardedConnections.map((connection) =>
      sanitizeConnectionForLog(connection, {
        discardReason: 'inactive',
        targetProjectId: connection.projectId || connection.targetProjectId || null,
      }),
    ),
    active: activeConnections.map((connection) =>
      sanitizeConnectionForLog(connection, {
        targetProjectId: connection.projectId || connection.targetProjectId || null,
      }),
    ),
